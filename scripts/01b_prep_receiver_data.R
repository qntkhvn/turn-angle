...
tracking_rec <- tracking_all |> 
  ...
  select(gameId, playId, frameId, 
         bc_id = receiver_id, bc_club = receiver_club,
...
tracking_def <- tracking_all |> 
  ...
  left_join(select(tracking_rec, gameId, playId, frameId,
...
tracking_cum_dis <- tracking |> 
  inner_join(distinct(tracking_angle_receivers, gameId, playId, nflId = bc_id)) |> 
